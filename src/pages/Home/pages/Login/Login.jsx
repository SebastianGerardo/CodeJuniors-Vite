...
const Login = () => {
  ...
  return (
    <div className="w-screen h-screen grid grid-cols-1 lg:grid-cols-[30%_70%]">
      <div className="hidden lg:grid  grid-rows-[30%_70%] text-center text-white bg-[#1E3A8A]">
        <div className="flex flex-col justify-center items-center max-w-[25rem] mx-auto">
            ...
            <h1 className="text-lg">Descubre todas las oportunidades laborales disponibles para ti</h1>
        </div>
      ...
      <div className="flex flex-col justify-center items-center">
        <form action="" onSubmit={(e) => handleSubmit(e)} className="relative px-4 py-12 flex flex-col items-start gap-4 w-max">
            ...
            <h2 className="flex gap-1 text-lg">
                ¿Eres nuevo? 
                <NavLink to={"/register"} className="text-[#FA632B] hover:text-[#FA632BD9] transition-all duration-150 font-medium" >Registrate Aquí</NavLink>
                 o 
            ...
            <div className="flex items-center gap-2">
                ...
                <div to="/" className="flex gap-2 items-center cursor-pointer bg-[#E1F0FF] text-[#3699FF] hover:bg-[#3699FF] hover:text-white h-12 transition-all duration-150 font-medium py-2 px-4 rounded">
                    <GoogleIcon />
                    Ingresa con Google
                </div>
            </div>
            <NavLink to={"/login_empresa"} className="text-[#3699FF] hover:text-[#0073E9] transition-all duration-150 font-normal text-sm">¿Eres Empresa?</NavLink>
        </form>
      </div>
    </div>
  );
...
export const InputDefault = ({label, type="text", name, placeholder, onChange}) => {
    return (
     <label className="w-full">
         <span className="pl-1 after:content-['*'] after:ml-0.5 after:text-red-500 block text-base font-medium text-slate-700">
             {label}
